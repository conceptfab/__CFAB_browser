...
class ImageProcessorApp(QMainWindow):
    ...
    def select_folder(self):
        ...
        folder = QFileDialog.getExistingDirectory(
            self, "Wybierz folder do przetworzenia"
        )
        if folder:
            self.selected_folder_path = folder
            self.folder_label.setText(f"Wybrano: {folder}")
            self.folder_label.setProperty("class", "folder-label-normal")
    ...
    def process_directory_tree(self, root_path: str):
        ...
        for dirpath, _, filenames in os.walk(root_path):
            ...
            zip_files = [
                f
                for f in filenames
                if f.lower().endswith(".zip") and "_model" not in f.lower()
            ]
                ...
                if len(jpg_files) == 1:
                    ...
                    if old_jpg_path != new_jpg_path:
                        try:
                            ...
                            self.log(
                                f"-> Zmieniono nazwę: '{jpg_filename}' -> '{new_name}'"
                            )
                        except Exception as e:
                            self.log(
                                f"-> BŁĄD przy zmianie nazwy pliku {jpg_filename}: {e}"
                            )
                    else:
                        self.log(
                            f"-> Plik '{jpg_filename}' ma już poprawną nazwę. Pomijam."
                        )

                # Przypadek 2: Jeden plik ZIP i wiele plików JPG
                elif len(jpg_files) > 1:
                    self.log(
                        f"-> Znaleziono {len(jpg_files)} plików JPG. Tworzę kompozycję..."
                    )
                    self.create_composite_image(dirpath, jpg_files, base_name)

            elif len(zip_files) > 1:
                self.log(
                    f"-> Pomijam folder. Znaleziono więcej niż jeden pasujący plik ZIP: {', '.join(zip_files)}"
                )
            else:
                self.log("-> W folderze brak pasujących plików ZIP lub JPG. Pomijam.")

    def create_composite_image(
        self, dirpath: str, jpg_files: list, output_base_name: str
    ):
        """
        ...
        try:
            ...
            if not images:
                self.log(
                    "  -> BŁĄD: Nie udało się wczytać żadnego obrazu do kompozycji."
                )
                return
            ...
            self.log(
                f"  -> Pomyślnie utworzono i zapisano kompozycję jako: '{output_filename}'"
            )
