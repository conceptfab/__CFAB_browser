# 📄 core/preview_window.py - Patch Code
...
- **Dodano klasę `ImageLoader`** - worker class do asynchronicznego ładowania obrazów
- **Implementacja `QThreadPool`** - zarządzanie wątkami w tle
- **Sygnały `image_loaded` i `image_scaled`** - komunikacja między wątkami

### 2. Optymalizacja Skalowania QPixmap

- **Pre-scaling obrazów** - skalowanie do maksymalnego rozmiaru ekranu przy ładowaniu
- **Debounced resize events** - opóźnienie 50ms przed skalowaniem w `resizeEvent`
- **Asynchroniczne skalowanie** - skalowanie w tle bez blokowania UI

### 3. Zarządzanie Pamięcią

- **Efektywne zarządzanie `QPixmap`** - czyszczenie zasobów w `closeEvent`
- **`pre_scaled_pixmap`** - przechowywanie wstępnie przeskalowanego obrazu
- **`waitForDone(1000)`** - oczekiwanie na zakończenie wątków w tle

### 4. Type Hints

- **Pełne adnotacje typów** - dla wszystkich metod i zmiennych
- **`Optional[QPixmap]`** - dla zmiennych, które mogą być None
- **Dokumentacja metod** - docstringi dla wszystkich funkcji
...
from PyQt6.QtCore import Qt, QThreadPool, QTimer, pyqtSignal, QObject, QSize
...
- ✅ **Podstawowa funkcjonalność:** Okno podglądu działa poprawnie
- ✅ **Kompatybilność API:** Zachowane wszystkie publiczne metody
- ✅ **Obsługa błędów:** Dodano proper error handling dla ładowania obrazów
- ✅ **Walidacja:** Sprawdzanie czy obraz został załadowany poprawnie
- ✅ **Logowanie:** Dodano logowanie błędów
- ✅ **Cache:** Implementacja pre-scaling jako forma cache'owania
- ✅ **Thread safety:** Asynchroniczne operacje z proper signal/slot communication
- ✅ **Wydajność:** Znacząca poprawa dzięki asynchronicznemu ładowaniu i pre-scaling

### Zależności

- ✅ **Importy:** Wszystkie importy działają poprawnie
- ✅ **Zależności zewnętrzne:** PyQt6.QtCore, PyQt6.QtGui, PyQt6.QtWidgets
- ✅ **Zależności wewnętrzne:** Brak cyklicznych zależności
...
- ✅ **Wydajnościowe:** Asynchroniczne ładowanie eliminuje blokowanie UI
...
1. **Eliminacja blokowania UI** - Obrazy ładują się asynchronicznie
2. **Szybsze skalowanie** - Pre-scaling eliminuje kosztowne operacje w resizeEvent
3. **Lepsze zarządzanie pamięcią** - Proper cleanup w closeEvent
4. **Debounced resize** - Eliminacja nadmiernego skalowania podczas resize
5. **Type safety** - Pełne type hints dla lepszej maintainability

## 📊 Metryki Wydajności

- **Czas ładowania:** Znacząco zmniejszony dzięki asynchroniczności
- **Responsywność UI:** Eliminacja zacięć podczas ładowania dużych obrazów
- **Zużycie pamięci:** Optymalizacja dzięki pre-scaling i proper cleanup
- **Thread safety:** Bezpieczne operacje wielowątkowe z signal/slot communication
