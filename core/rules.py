...
class FolderClickRules:
    ...
    def _handle_condition_1(folder_path: str, content: Dict) -> Dict:
        ...
        logger.debug(
            f"CONDITION 1 DETECTED: {folder_path} | "
    ...
    def _handle_condition_2a(folder_path: str, content: Dict) -> Dict:
        """
        Handles condition 2a
        """
        asset_count = content["asset_count"]
        preview_archive_count = content["preview_archive_count"]

        logger.debug(
            f"CONDITION 2A DETECTED: {folder_path} | "
            f"Archive/Preview files: {preview_archive_count} | "
            f"Asset files: {asset_count} | "
            f"Cache: NO | "
            f"DECISION: Running scanner (no .cache folder)"
        )

        return {
            "action": "run_scanner",
            "message": "No .cache folder - running scanner",
    ...
    def _handle_condition_2b(folder_path: str, content: Dict) -> Dict:
        ...
        logger.debug(
            f"CONDITION 2B DETECTED: {folder_path} | "
            f"Archive/Preview files: {preview_archive_count} | "
            f"Asset files: {asset_count} | "
            f"Cache: YES | "
            f"Thumbnails: {cache_thumb_count} | "
            f"DECISION: Running scanner (mismatched number of thumbnails)"
        )

        return {
            "action": "run_scanner",
            "message": (
                f"Mismatched number of thumbnails ({cache_thumb_count}) "
                f"and asset files ({asset_count}) - running scanner"
            ),
    ...
    def _handle_condition_2c(folder_path: str, content: Dict) -> Dict:
        ...
        logger.debug(
            f"CONDITION 2C DETECTED: {folder_path} | "
            f"Archive/Preview files: {preview_archive_count} | "
            f"Asset files: {asset_count} | "
            f"Cache: YES | "
            f"Thumbnails: {cache_thumb_count} | "
        ...
        return {
            "action": "show_gallery",
            "message": (
                f"All ready - displaying gallery "
    ...
    def _handle_additional_case_no_cache(folder_path: str, content: Dict) -> Dict:
        """
        Handles additional case: Only asset files, no .cache
        """
        asset_count = content["asset_count"]
        preview_archive_count = content["preview_archive_count"]
        ...
        logger.debug(
            ...
            f"Archive/Preview files: {preview_archive_count} | "
            f"Cache: NO | "
            f"DECISION: Running scanner (only asset, no .cache)"
        )

        return {
            "action": "run_scanner",
            "message": "Only asset files, no .cache - running scanner",
    ...
    def _handle_additional_case_mismatch(folder_path: str, content: Dict) -> Dict:
        """
        Handles additional case: Only asset files, mismatched number of thumbnails
        """
        asset_count = content["asset_count"]
        preview_archive_count = content["preview_archive_count"]
        cache_thumb_count = content["cache_thumb_count"]
    ...
    def _handle_additional_case_ready(folder_path: str, content: Dict) -> Dict:
        """
        Handles additional case: Only asset files, all ready
        """
        asset_count = content["asset_count"]
        preview_archive_count = content["preview_archive_count"]
        cache_thumb_count = content["cache_thumb_count"]
    ...
    def _handle_default_case(folder_path: str, content: Dict) -> Dict:
        """
        Handles default case: Folder does not contain appropriate files
        """
        asset_count = content["asset_count"]
        preview_archive_count = content["preview_archive_count"]
