...
class PerformanceMonitor:
    ...
    def measure_function(
        ...
        def decorator(func: Callable) -> Callable:
            def wrapper(*args, **kwargs):
                ...
                with self.measure_operation(op_name, additional_data) as metrics:
                    result = func(*args, **kwargs)
                    # Dodaj informacje o wyniku jeśli to możliwe
                    if hasattr(result, "__len__"):
                        metrics.additional_data["result_size"] = len(result)
                    return result
    ...
    def get_statistics(
        ...
        Args:
            operation_name: Nazwa operacji do analizy (None = wszystkie)
            limit: Maksymalna liczba ostatnich wpisów do analizy
        ...
        # Dodaj statystyki pamięci jeśli dostępne
        memory_after = [m.memory_after for m in metrics if m.memory_after is not None]
        if memory_after:
            stats.update(
                {
                    "avg_memory_after_mb": sum(memory_after) / len(memory_after),
                    "max_memory_after_mb": max(memory_after),
                    "min_memory_after_mb": min(memory_after),
                }
            )
