...
class AssetTileModel(QObject):
    ...
    def get_thumbnail_path(self) -> str:
        ...
        if not self.data.get("thumbnail"):
            return ""
        # Oblicz folder_path z lokalizacji pliku .asset
        folder_path = self._get_folder_path_from_file()
        if not folder_path:
            return ""
        cache_dir = os.path.join(folder_path, ".cache")
        return os.path.join(cache_dir, f"{self.get_name()}.thumb")

    def _get_folder_path_from_file(self) -> str:
        """Oblicza folder_path z lokalizacji pliku .asset"""
    ...
    def get_folder_path(self) -> str:
        ...
        return self._get_folder_path_from_file()
